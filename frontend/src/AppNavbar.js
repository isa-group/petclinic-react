...
import { Default, ErrorFallback, Feature, On, Loading } from "pricing4react";
import { feature } from "pricing4react";
...
function AppNavbar() {
    ...
    if (role === "ADMIN") {
      adminLinks = (
        <>
          ...
          <NavItem>
            <NavLink style={{ color: "white" }} tag={Link} to="/plansAdmin">
              Plans
            </NavLink>
          </NavItem>
          <NavItem>
  ...
  if (!jwt) {
    ...
    userLinks = (
      <>
        <Feature>
          <On expression={feature("havePetsDashboard")}>
            <NavItem>
